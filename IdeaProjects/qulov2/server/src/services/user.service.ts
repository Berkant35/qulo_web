...
export class UserService {
  ...
  async updateProfile(userId: string, data: UpdateProfileInput) {
    const { data: user, error } = await supabase
      ...
      .eq("is_deleted", false)
      .select(
        "id, email, name, surname, bio, age, gender, gender_pref, match_radius_km, age_pref_min, age_pref_max, city, country, locale, lat, lng, photos, profile_completion, diamonds, green_diamonds, is_online, last_seen, email_verified, created_at, updated_at",
      )
      .maybeSingle();

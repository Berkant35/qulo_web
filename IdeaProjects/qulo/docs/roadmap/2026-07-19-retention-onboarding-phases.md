...
- (2026-08-01 Faz 2, guard 2. tur — **Y3**) `POWER_UNBLOCK` (chat) ve `rescueWithSkip`
  ...
  degil; ayri turda migration 037'deki RPC'lerle temiz baglanmali.
...
| 2026-08-01 | Faz 2 | ✅ **TAMAMLANDI + YAYINLANDI.** Cihaz testleri gecti. Android **Play production'da CANLI** (2.0.7+69, 10 dil release notu); iOS App Store'da PREPARE_FOR_SUBMISSION, build 69 bagli, What's New 21 dilde — "Add for Review" bekliyor. Migration 036+037 uygulandi. **Ayni surume giren ekstra isler:** purchases_flutter 8.11→10.7 (Play Billing 8.3.0, Google'in 31 Agu 2026 zorunlulugu), Smart App Banner (quloapp.com), AASA yollari Android ile esitlendi, sunucudaki olu .well-known route'lari silindi, Play icin fastlane kuruldu (service account + 10 dil changelog, dil listesi API'den canli). **Iki prod olayi:** (1) guard beklenmeden push → chat gucleri bir sure 500 dondu (JSONB vs text[] tip farki), migration 037 ile duzeldi; (2) terfi lane'inde track_promote_release_status verilmedigi icin 2.0.7 istemeden Play'de canliya cikti. Ikisi de memory'ye ders olarak yazildi. | 2.0.7 | **Kapsam degisti:** 2.3 IPTAL (kullanici: "basarisizsa basarisizdir"), yerine 2.1b starter paket (2x ORACLE envanter) + 2.1c cift-ucretlendirme korumasi geldi; chat de kapsama alindi. **Uc kesif duzeltmesi:** (1) chat "dogru referans" degilmis — fiyati gosterip tap'i engelliyordu, yani sunucudaki elmas yolu prod'da HIC calismamis; (2) 2.2 cift bug — carpan kaybi + kaynak ayrisimi (rescue `powers.base_cost`, normal kullanim `config.powerCosts`); (3) envanter kapisi bir cift-ucretlendirme deligini kapatiyormus, kaldirinca ust sinir kullanici bakiyesi oluyordu (chat-flow-guard BLOCKER). Ekonomi: YUKSEK (8), mor elmas ekonomisi degismiyor (starter paket envanter, fungible degil). Commit: qulo-server af54891, qulov2 96759df. |

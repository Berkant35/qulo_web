...
class _CustomRateWidgetState extends ConsumerState<CustomRateWidget> {
  ...
  TextButton _sendButton(BuildContext context, AppLocalizations inst) {
    return TextButton(
        onPressed: () async {
          await FirebaseIgaCollectionEnumsWithField.iga_rates.reference.add({
            'rating': _rating,
            'comment': _controller.text,
            'created_at': DateTime.now(),
          });
        ...
        child: Text(
          inst.form_send,
          style: const TextStyle(color: CpColors.cpFrenchLime, fontSize: 20),
        ));
  }

  TextField _feedbackTextfield(AppLocalizations inst) {
    return TextField(
        controller: _controller,
        decoration: InputDecoration(
            contentPadding:
            ...
            border: OutlineInputBorder(
              borderSide:
                  const BorderSide(color: CpColors.cpFrenchLime, width: 1),
              borderRadius: BorderRadius.circular(30),
            ),
            enabledBorder: OutlineInputBorder(
              borderSide:
  ...
  AnimatedRatingStars _rateStars() {

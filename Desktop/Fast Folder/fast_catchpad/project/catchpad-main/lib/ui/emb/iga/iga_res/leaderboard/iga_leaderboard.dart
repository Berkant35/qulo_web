...
class _IgaLeaderboardPageState extends ConsumerState<IgaLeaderboardPage>
    ...
    Future(() {
      ref.read(currentIgaLeaderboardManager.notifier).changeState(
          ...
          ref.read(currentGameProv) != null
              ? tabGameIds[
                  tabGameIds.indexOf((ref.read(currentGameProv)?.id ?? 's16'))]
              : ref
                          .read(currentIgaResultManager.notifier)
                          .currentLocation
                          ?.igaLastGameInfoId !=
                      null
                  ? tabGameIds[tabGameIds.indexOf((ref
                      .read(currentIgaResultManager.notifier)
                      .currentLocation!
                      .igaLastGameInfoId)!)]
                  : tabGameIds[0]);

      if (ref.read(currentGameProv) != null) {
        index = tabGameIds.indexOf((ref.read(currentGameProv)!.id));
      } else if (ref
              .read(currentIgaResultManager.notifier)
              .currentLocation
              ?.igaLastGameInfoId !=
          null) {
        index = tabGameIds.indexOf((ref
            .read(currentIgaResultManager.notifier)
            .currentLocation!
            .igaLastGameInfoId)!);
      } else {
      ...
      tabController.animateTo(
          tabGameIds.indexOf((ref.watch(currentIgaLeaderboardManager))!));

      ref.read(currentIgaLeaderboardManager);
      setState(() {});
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    Future(() {
      if (ref.read(currentIgaTraceStateManager) ==
          IgaPlayTraceStates.register) {
        ref
    ...
    // if (ref.read(currentGiveRateManager)) IgaDialogs.giveRateDialog(context,ref);
  ...
  Column leaderboard(double width, double height, BuildContext context,
      double firstWidth, double firstHeight, int selected) {
    return Column(
      children: [
        Row(
          ...
          children: [
            Expanded(
              flex: 13,
              child: _leaderboard(
                  width, height, context, firstWidth, firstHeight, selected),
            ),
            IgaLastGameUser(
                selectedGameId:

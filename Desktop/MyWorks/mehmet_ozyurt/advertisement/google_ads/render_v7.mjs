...
const outputDir = path.join(__dirname, 'output7');
if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });
const templateMap = { square: 'templates/square_v7.html', portrait: 'templates/portrait_v7.html', landscape: 'templates/landscape_v7.html' };
async function run() {
  ...
  for (const ad of config.ads) {
    ...
    console.log(`[${sizeKB > 500 ? 'ERR' : sizeKB > 150 ? 'WARN' : 'OK'}] Ad ${ad.id}: ${ad.output_name}.png (${sizeKB}KB)`);
    await page.close();
  }
  await browser.close();
}

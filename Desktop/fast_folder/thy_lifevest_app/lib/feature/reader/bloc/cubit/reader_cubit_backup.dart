...
class ReaderCubit extends Cubit<ReaderState> {
    ...
    try {
      ...
      switch (command) {
        ...
        case 0x8D: // Inventory stop response (0x8C + 1)
          ...
          break;
        case 0x6F: // Filter response (0x6E + 1)
  ...
  Future<void> startInventory({int maxEmptyCount = 5}) async {
    try {
      ...
      // Önce mevcut inventory'yi durdur
      await stopInventory();
      ...
      if (!success) {
        throw Exception('Failed to send inventory start command');
      }
  ...
  Future<bool> stopInventory() async {
    try {
      debugPrint("[READER] Stopping inventory...");

      // 1. Önce timer'ı ve loop'u durdur
      _stopInventoryTimer();

      // 3. Stop komutu gönder ve response bekle
      final command = _buildCommand(0x8C, null);
      final response = await _sendCommandAndGetResponse(command);

      if (response != null && response.isNotEmpty) {
        final success = response[0] == 0x01;

        if (success) {
          debugPrint("[READER] Inventory stopped successfully");
        } else {
          debugPrint("[READER] Inventory stop command returned error: ${response[0]}");
        }

        return success;
      } else {
        debugPrint("[READER] Inventory stop command: no response received");
        return false;
      }
    } catch (e) {
  ...
  Future<void> startFindMode(String targetEpc) async {
    try {
      ...
      debugPrint("[READER] Find mode started");
    } catch (e) {
      emit(state.setFailure(AppReaderConstants.readFailedCode, AppStrings.error, 'Failed to start find mode: $e'));
    }
  }
  ...
  Future<void> stopFindMode() async {
    ...
    await stopInventory();
    debugPrint("[READER] Find mode stopped");
  }
  ...
  void _startInventoryLoop(int maxEmptyCount) {
    ...
    _inventoryTimer = Timer.periodic(const Duration(milliseconds: 50), (timer) async {
  ...
  void _parseTagDataFromBuffer(Uint8List data, int reportedTagCount) {
    try {
      debugPrint("[READER] Parsing $reportedTagCount tags from buffer, data length: ${data.length}");
      debugPrint("[READER] Tag buffer hex data: ${_bytesToHex(data)}");
      ...
      for (int i = 0; i < reportedTagCount; i++) {
        if (mainCursor >= data.length) {
          debugPrint("[READER] Buffer exhausted at tag $i, cursor: $mainCursor");
          break;
        }

        final rawLen = data[mainCursor];
        debugPrint("[READER] Tag ${i + 1}: rawLen = $rawLen at cursor $mainCursor");
        mainCursor++;

        if (rawLen == 0 || mainCursor + rawLen > data.length) {
          debugPrint("[READER] Invalid tag length at tag $i: $rawLen, remaining: ${data.length - mainCursor}");
          break;
        }

        final rawTag = data.sublist(mainCursor, mainCursor + rawLen);
        debugPrint("[READER] Tag ${i + 1} raw data: ${_bytesToHex(rawTag)}");
        mainCursor += rawLen;
  ...
  ReaderTag? _parseRawTag(Uint8List rawTag) {
    try {
      ...
      debugPrint("[READER] Parsing raw tag: $hexRawTag");

      // PC word (Protocol Control) - first 2 bytes
      final pc = _bytesToHex(rawTag.sublist(0, 2));
      final epcLen = rawTag[0] >> 3; // EPC length in words

      debugPrint("[READER] PC: $pc, EPC length: $epcLen words (${epcLen * 2} bytes)");

      String? epc;
      if (2 + epcLen * 2 <= rawTag.length) {
        epc = _bytesToHex(rawTag.sublist(2, 2 + epcLen * 2));
        debugPrint("[READER] EPC: $epc");
      } else {
        debugPrint("[READER] ⚠️ Not enough data for EPC: expected ${epcLen * 2} bytes, got ${rawTag.length - 2}");
      }
      ...
      if (rawTag.length >= 2) {
        ...
        debugPrint("[READER] RSSI raw: 0x${rssiRaw.toRadixString(16)}, calculated: ${rssi} dBm");
      }

      if (epc != null && epc.isNotEmpty) {
        final tag = ReaderTag(pc: pc, epcLen: epcLen, epc: epc, rssi: rssi, readTime: DateTime.now());

        debugPrint("[READER] ✅ Successfully created ReaderTag: ${tag.epc}");
        return tag;
      } else {
        debugPrint("[READER] ❌ Invalid EPC data");
        return null;
      }
    } catch (e) {
  ...
  Map<String, dynamic> decodeGs1AdiEpc(String epcHex) {
    try {
      ...
      final bytes = <int>[];
      debugPrint("[READER] Decoding EPC: $epcHex");
      ...
      debugPrint("[READER] ✅ EPC decoded successfully: $result");
      return result;
    } catch (e) {
      return {
        'error': e.toString(),
  ...
  String decodeGs16Bit(String bits) {
    try {
      ...
      debugPrint("[READER] GS1 16-bit decoded: '$result'");
      return result;
    } catch (e) {
